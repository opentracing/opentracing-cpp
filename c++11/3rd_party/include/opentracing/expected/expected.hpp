...
#define nsel_REQUIRES_T(...)                 \
  typename =                                 \
      typename std::enable_if<(__VA_ARGS__), \
                              opentracing::expected_detail::enabler>::type
...
union storage_t {
 ...
 private:
  ...
  void construct_value(value_type const &v) { new (&m_value) value_type(v); }
  ...
  value_type *value_ptr() const { return &m_value; }
...
class unexpected_type {
 public:
  ...
  nsel_REQUIRES_0(std::is_copy_constructible<error_type>::value)

      constexpr explicit unexpected_type(error_type const &error)
      : m_error(error) {}

  nsel_REQUIRES_0(std::is_move_constructible<error_type>::value)

      constexpr explicit unexpected_type(error_type &&error)
      : m_error(std::move(error)) {}

  constexpr error_type const &value() const { return m_error; }

  error_type &value() { return m_error; }
...
class bad_expected_access : public std::logic_error {
 public:
  ...
  explicit bad_expected_access(error_type error)
      : logic_error("bad_expected_access"), m_error(error) {}
...
class expected {
 public:
  ...
  nsel_constexpr14 expected(expected const &rhs) : has_value_(rhs.has_value_) {
    if (has_value())
  ...
  nsel_constexpr14 expected(expected &&rhs) : has_value_(rhs.has_value_) {
    if (has_value())
  ...
  nsel_REQUIRES_0(std::is_copy_constructible<T>::value)
  ...
  nsel_REQUIRES_0(std::is_move_constructible<T>::value)
  ...
  template <typename... Args,
            nsel_REQUIRES_T(std::is_constructible<T, Args &&...>::value)>
  ...
  template <typename U, typename... Args,
            nsel_REQUIRES_T(std::is_constructible<T, std::initializer_list<U>,
                                                  Args &&...>::value)>

  nsel_constexpr14 explicit expected(in_place_t, std::initializer_list<U> il,
                                     Args &&... args)
      : has_value_(true) {
    contained.construct_value(il, std::forward<Args>(args)...);
  }

  nsel_REQUIRES_0(std::is_copy_constructible<E>::value)
  ...
  nsel_REQUIRES_0(std::is_move_constructible<E>::value)
  ...
  template <typename... Args,
            nsel_REQUIRES_T(std::is_constructible<E, Args &&...>::value)>
  ...
  nsel_constexpr14 explicit expected(in_place_unexpected_t,
  ...
  template <typename U, nsel_REQUIRES_T(std::is_constructible<T, U>::value &&
    ...
    return has_value() ? contained.value()
                       : std::is_same<error_type, std::exception_ptr>::value
                             ? (std::rethrow_exception(contained.error()),
                                contained.value())
                             : (throw bad_expected_access<error_type>(
                                    contained.error()),
                                contained.value());
  }
  ...
  bool has_exception() const {
    return !has_value() &&
           std::is_base_of<Ex, decltype(get_unexpected().value())>::value;
  }

  template <typename U,
            nsel_REQUIRES_T(std::is_copy_constructible<T>::value
  ...
  template <typename U,
            nsel_REQUIRES_T(std::is_move_constructible<T>::value
                                &&std::is_convertible<U &&, T>::value)>
...
class expected<void, E> {
 public:
  ...
  constexpr explicit expected(in_place_t) : has_value_(true) {}
