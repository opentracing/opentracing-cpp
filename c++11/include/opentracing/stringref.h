...
inline StringRef::operator const char*() const { return m_data; }

inline void StringRef::reset(const char* str, const size_t length) {
  m_data = str;
  m_length = length;
}

template <size_t N>
void StringRef::reset(const char (&str)[N]) {
  m_data = str;
  m_length = N;
}

inline void StringRef::reset(const char* str) {
  m_data = str;
  m_length = std::strlen(str);
}

inline void StringRef::reset(const std::basic_string<char>& str) {
  m_data = str.data();
  m_length = str.length();
}

inline const char* StringRef::data() const { return m_data; }

inline size_t StringRef::length() const { return m_length; }
